...
var Connection = function(proxy, connection, logger) {
  ...
  MongoClient.connect(proxy.options.uri, {
    ...
    } else if(self.db.serverConfig instanceof m.Mongos) {
...
Connection.prototype.replyWithMaster = function (requestId, responseTo) {
    var self = this;
...
Connection.prototype.replyWithError = function (requestId, responseTo, message) {
    var self = this;
    if (self.logger.isDebug())
    ...
    // Create a new Message Response and reply to the ismaster
    var reply = new ReplyMessage(self.topology.bson, requestId, responseTo, [errorResponse]);
    // Write it to the connection
    try {
        self.connection.write(reply.toBin());
    } catch (err) {
...
Connection.prototype.messageHandler = function(data) {
  ...
  // Get the request Id
  var message = new WireMessage(data,self.topology.bson);
  ...
  if (bufferIndexOf(data, ismaster, 0) !== -1) {
    ...
    // Client message
    var clientMessage = message; //new WireMessage(data);
    console.log("=====>:"+clientMessage);
    var server = null;

    //We have a OP_GETMORE pick the right server callback pool
    if (clientMessage.opCode() === 2005) {
      var cname = clientMessage.getMoreCursorID().toString();
      console.log("OP_GETMORE from pool: " +cname + " with "+self.connections[cname]);
      if (cname && self.connections[cname]) {
          ...
          self.connections[cname] = {
              conn:  server.getConnection(), server: server
          };
      }

    } else if (opCode === 2001 || opCode === 2002 || opCode === 2006) {
        //nop
        //insert, delete and update won't work
        console.log("!!!!! PROTECTED COMMAND !!!!! will skip =>"+opCode);
        self.replyWithError(requestId,responseTo,"Cannot execute protected opCode "+opCode);
        return;
    } else {
        if (opCode === 2004) {
           var q = clientMessage.getQuery();
           for (var j=0; j<blacklist.length; j++) {
               if (q.query[blacklist[j]]) {
    ...
    var callbackFunction = function(_server, _clientMessage, _clientConnection) {
      ...
      if(clientMessage.opCode() === 2005) {
        ...
        console.log("  unpacked cursor:"+curs.toString());
      ...
      return function(err, data) {
        ...
        var responseMessage = new WireMessage(data.raw, self.topology.bson);
        ...
        if (cursorID.isZero()
            ...
            var connId = clientMessage.getMoreCursorID();
          ...
          self.connections[cursorID] = {
              conn: connection, server: _server
          }
        }
  ...
  return function(data) {
    ...
    while(data.length > 0) {
      ...
      if(self.bytesRead > 0 && self.sizeOfMessage > 0) {
        ...
        if(self.stubBuffer && self.stubBuffer.length > 0) {
